...
fprintf(1, '- s: save figure as .svg to working directory\n')



params.smoothSize = 1.6666; % in msec, stdev of gaussian smoothing filter. 6 x std. dev is the size of the window
...
for g = 1:nGroups
    x_end = max(rasterX);
    new_y = prev_y+sum(myData.trGroups==g);
    hold on; plot([x_end x_end], [prev_y prev_y+sum(myData.trGroups==g)],'color', colors(g,:))';
    prev_y = new_y;
end
xlim(myData.params.window);
...
switch keydata.Key
    ...
    case 's'
        saveas(gcf,['cluster ' num2str(myData.clusterIDs(myData.params.clusterIndex))],'png');
end
